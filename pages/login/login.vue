...
<script>
	...
	export default {
		...
		methods: {
			...
			//发送验证码
			sendCode() {
				console.log(this.forgetFormData)
				if(this.forgetFormData.phone==="")
				{
					uni.showToast({
						title:'请填写手机号',
						icon:'none',
				...
				let _this = this
				uni.request({
					url: _this.apiServer + '/api/sms/send',
					data: {
						phone: _this.forgetFormData.phone
					},
					header: {
						'content-type': 'application/x-www-form-urlencoded',
						'enctype': "multipart/form-data"
			...
			submitForget(forgetForm) {
				this.$refs.forgetForm.validate().then(res => {
					...
					if(this.forgetFormData.newPwd === this.forgetFormData.rePwd)
					{
						let _this = this
						uni.request({
							url: _this.apiServer + '/api/sms/verify',
							data: {
								phone:_this.forgetFormData.phone,
								code:_this.forgetFormData.securityCode,
								password:_this.forgetFormData.newPwd
							},
							header: {
								'content-type': 'application/x-www-form-urlencoded',
								'enctype': "multipart/form-data"
