<template>
	...
	<view class="content">
		<view v-for="(item,index) in homePosts" :key="index" class="card">
			<uni-card>
				<navigator :url="'/vetPages/examine/examineDetail/returnDetail?returnId='+ item.returnId+'&fromPage=examToDo'" >
				...
				<view class="uni-flex uni-row">
					...
					<view class="uni-flex uni-column">
						...
						<view class="uni-flex uni-column">
							...
							<text>申报时间：{{new Date(parseInt(item.protectTime)).toLocaleString().replace(/:\d{1,2}$/,' ')}}</text>
							<text>申报编号：{{item.protectId}}</text>
