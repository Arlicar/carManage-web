	...
	<view class="content">
		...
		<view class="main">
			<view class="uni-flex uni-column forms">
				<text class="intro">维修单编号：{{list.lendId}}</text>
				<text class="intro">维修车辆车牌：{{list.returnLicense}}</text>
				<text class="intro">维修说明：{{list.returnMsg}}</text>
				<text class="intro">维修部位照片：</text>
				<image class="img" :src="list.returnImg"></image>
				<text class="intro">维修时间：{{new Date(parseInt(list.returnTime)).toLocaleString().replace(/:\d{1,2}$/,' ')}}</text>
				<text class="intro">勘察员工号：{{list.surveyNo}}</text>
				<text class="intro">勘察员操作时间：{{new Date(parseInt(list.returnTime)).toLocaleString().replace(/:\d{1,2}$/,' ')}}</text>
			</view>
							...
							success(res) {
								...
								if (res.data.code == 200) {
									if (_this.src === 1) {
										...
										uni.request({
											url: _this.apiServer + "/api/update/return",
											method: 'POST',
											header: {
												'content-type': 'application/x-www-form-urlencoded',
												'enctype': "multipart/form-data"
