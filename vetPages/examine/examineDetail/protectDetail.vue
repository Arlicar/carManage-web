<template>
	<view class="content">
		...
		<!-- 维修审批 -->
		<view class="main">
			<view class="uni-flex uni-column forms" v-for="item in list" key="index">
				<text class="intro">保养单编号：{{item.protectId}}</text>
				<text class="intro">保养车辆车牌：{{item.protectLicense}}</text>
				<text class="intro">保养说明：{{item.protectMsg}}</text>
				<text class="intro">保养部位照片：</text>
				<image class="img" :src="item.protectImg"></image>
				<text
					class="intro">保养时间：{{new Date(parseInt(item.protectTime)).toLocaleString().replace(/:\d{1,2}$/,' ')}}</text>
				<text class="intro">管理员工号：{{item.manageNo}}</text>
				<text
					class="intro">操作时间：{{new Date(parseInt(item.manageTime)).toLocaleString().replace(/:\d{1,2}$/,' ')}}</text>
			</view>

			<view class="uni-flex uni-column forms" v-if="fromPage=='examDone'" style="width: 30%;
    margin: 30rpx auto;">
				<text class="intro pass" v-if="item.vetResult==1">通过</text>
