	...
	<view class="content">
		...
		<view class="main">
			<view class="uni-flex uni-column forms">
				<text class="intro">借车单编号：{{lendId}}</text>
				<text class="intro">借车车辆车牌：{{list.lendLicense}}</text>
				<text class="intro">借车说明：{{list.lendMsg}}</text>
				<text class="intro">驾照照片：</text>
				<image class="img" :src="list.lendDrivecardImg"></image>
				<text
					class="intro">借车时间：{{new Date(parseInt(list.lendTime)).toLocaleString().replace(/:\d{1,2}$/,' ')}}</text>
				<text class="intro">管理员工号：{{list.manageNo}}</text>
				<text
					class="intro">操作时间：{{new Date(parseInt(list.manageLendTime)).toLocaleString().replace(/:\d{1,2}$/,' ')}}</text>
			</view>

			<view class="uni-flex uni-column forms" v-if="fromPage=='examDone'" style="width: 30%;
    margin: 30rpx auto;">
				<text class="intro pass" v-if="list.vetLendResult==1">通过</text>
							...
							success(res) {
								...
								if (res.data.code == 200) {
									if (_this.src === 1) {
										...
										uni.request({
											url: _this.apiServer + "/api/update/vet/lend",
											method: 'POST',
											header: {
												'content-type': 'application/x-www-form-urlencoded',
												'enctype': "multipart/form-data"
