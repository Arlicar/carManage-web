	...
	<view class="content">
		...
		<view class="main">
			<view class="uni-flex uni-column forms">
				<text class="intro">派车单编号：{{list.assignId}}</text>
				<text class="intro">派车车辆车牌：{{list.assignLicense}}</text>
				<text class="intro">派车说明：{{list.assignMsg}}</text>
				<text class="intro">派车时间：{{new Date(parseInt(list.lendTime)).toLocaleString().replace(/:\d{1,2}$/,' ')}}</text>
				<text class="intro">勘察员工号：{{list.surveyNo}}</text>
				<text class="intro">管理员工号：{{list.manageNo}}</text>
				<text class="intro">操作时间：{{new Date(parseInt(list.manageTime)).toLocaleString().replace(/:\d{1,2}$/,' ')}}</text>
				<text class="intro">负责人工号：{{list.vetNo}}</text>
				<text class="intro">操作时间：{{new Date(parseInt(list.vetTime)).toLocaleString().replace(/:\d{1,2}$/,' ')}}</text>
			</view>
			
			<view class="uni-flex uni-column forms" v-if="fromPage=='examDone'" style="width: 30%;
    margin: 30rpx auto;">
				<text class="intro pass" v-if="list.result==1">通过</text>
	...
	export default {
		...
		methods: {
			...
			getList() {
				let _this = this;
				uni.request({
					url: _this.apiServer + "/api/get/getDetail/assign",
					method: 'POST',
					header: {
							...
							success(res) {
								...
								if (res.data.code == 200) {
									if (_this.src === 1) {
										...
										uni.request({
											url: _this.apiServer + "/api/update/vet/assign",
											method: 'POST',
											header: {
												'content-type': 'application/x-www-form-urlencoded',
												'enctype': "multipart/form-data"
											...
											data: {
												assignId: _this.assignId,
												result: 1,
												vetTime:_this.list.vetTime,
												position:_this.$store.state.userInfo.position,
												vetNo:_this.list.vetNo,
												resultMsg:'',
											},
											success: res => {
												if(res.data.code==200){
