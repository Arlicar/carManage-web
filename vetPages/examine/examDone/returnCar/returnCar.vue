<template>
	...
	<view class="content" :style="'height:'+totalHeight+'%'">
		<view v-for="(item,index) in homePosts" :key="index" class="card">
			<uni-card>
				<navigator :url="'/managePages/examine/examineDetail/returnDetail?returnId='+ item.returnId+'&fromPage=examDone'" >
				...
				<view class="uni-flex uni-row">
					...
					<view class="uni-flex uni-column">
						<view style="margin-top: 10rpx;">
							<text>{{item.protectLicense}}</text>
						</view>
						<view class="uni-flex uni-column">
							<text>申报人：{{item.name}}</text>
							<text>还车时间：{{new Date(parseInt(item.protectTime)).toLocaleString().replace(/:\d{1,2}$/,' ')}}</text>
							<text>还车编号：{{item.protectId}}</text>
	...
	export default {
		...
		methods: {
			getHomePosts() {
				let _this = this;
				uni.request({
					url: _this.apiServer + "/api/get/getAll/examin/examDone/protect",
					header: {
