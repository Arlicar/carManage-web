<template>
	...
	<view class="content" :style="'height:'+totalHeight+'%'">
		<view v-for="(item,index) in homePosts" :key="index" class="card">
			<uni-card>
				<navigator :url="'/vetPages/examine/examineDetail/lendDetail?lendId='+ item.lendId+'&fromPage=examDone'" >
				...
				<view class="uni-flex uni-row">
					...
					<view class="uni-flex uni-column">
						...
						<view class="uni-flex uni-column">
							...
							<text>借车时间：{{new Date(parseInt(item.lendTime)).toLocaleString().replace(/:\d{1,2}$/,' ')}}</text>
							<text>借车编号：{{item.lendId}}</text>
