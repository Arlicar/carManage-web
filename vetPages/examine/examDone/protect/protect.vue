<template>
	...
	<view class="content" :style="'height:'+totalHeight+'%'">
		<view v-for="(item,index) in homePosts" :key="index" class="card">
			<uni-card>
				<navigator :url="'/vetPages/examine/examineDetail/protectDetail?protectId='+ item.protectId+'&fromPage=examDone'" >
				...
				<view class="uni-flex uni-row">
					...
					<view class="uni-flex uni-column">
						...
						<view class="uni-flex uni-column">
							...
							<text>保养时间：{{new Date(parseInt(item.protectTime)).toLocaleString().replace(/:\d{1,2}$/,' ')}}</text>
							<text>保养编号：{{item.protectId}}</text>
