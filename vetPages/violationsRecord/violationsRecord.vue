	...
	<view class="content" :style="'height:'+contentHeight+'%'">
		...
		<view style="position: absolute;top: 380rpx;left: 3rpx;width: 740rpx;">
			<view v-for="(item,index) in productList" :key="index" class="uni-flex uni-column card"
				...
				<navigator url="../violationsRecord/violationDetail/violationDetail">
					<view style="font-weight: bold;margin-left: 30rpx;">
						<text>违规信息:{{item.accidentMsg}}</text>
					</view>
					<view style="color: #999999;margin-left: 30rpx;">
						<text>管理员编号:{{item.manageNo}}</text>
						<text v-if="item.payStatus==='1'" style="color: #478df3;margin-left: 50rpx;">支付状态:已处理
						</text>
						<text v-if="item.payStatus==='0'" style="color: #f92672;margin-left: 50rpx;">支付状态:未处理
						</text>
					</view>
					<view class="uni-flex uni-row"
						style="color: #999999;margin-left: 30rpx;justify-content: space-between; margin-right: 60rpx;">
						<text>完成时间:{{item.doneTime}}</text>
						<text>车牌号:{{item.license}}</text>
					</view>
				</navigator>
			</view>
		</view>
		...
		<!-- <view style="position: absolute;top: 380rpx;left: 3rpx;width: 740rpx;">
			<view v-for="(item,index) in productList" :key="index" class="uni-flex uni-column card">
				<view class="uni-flex uni-row"
					...
					<text v-if="item.payStatus==='1'" style="color: #478df3;top: 100rpx;left: 10rpx;">支付状态:已处理
					</text>
					<text v-if="item.payStatus==='0'" style="color: #f92672;top: 100rpx;left: 10rpx;">支付状态:未处理
					</text>
				</view>
				<view class="uni-flex uni-row"
					style="margin-left: 30rpx;justify-content: space-between; margin-right: 20rpx;">
					<text style="color: #999999;top: 100rpx;left: 370rpx;width: 400rpx;">管理员:{{item.manageNo}}</text>
				</view>
				<view class="uni-flex uni-row"
					style="margin-left: 30rpx;justify-content: space-between; margin-right: 20rpx;">
					<text style="color:	 #999999;top: 60rpx;left: 10rpx;">车牌号:{{item.license}}</text>
				</view>
