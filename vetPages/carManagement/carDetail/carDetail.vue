		...
		<view class="uni-flex uni-column">
			<view class="introBody" v-for="(item,index) in productList" :key="index">
				...
				<view class="uni-flex uni-row">
					...
					<view v-if="item.status==0" class="state" style="color: #ffc940;font-weight:600;">
						<text>已派出</text>
					</view>
					<view v-if="item.status==1" class="state" style="color: #87e9ff;font-weight:600;">
						<text>未派出</text>
					</view>
					<view v-if="item.status==2" class="state" style="color: #d295de;font-weight:600;">
						<text>维修中</text>
					</view>
					<view v-if="item.status==3" class="state" style="color: #7cff87;font-weight:600;">
						<text>保养中</text>
					</view>
					<view v-if="item.status==4" class="state" style="color: #a25454;font-weight:600;">
						<text>出借中</text>
					</view>
				</view>
				</uni-card>
