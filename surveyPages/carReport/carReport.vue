<template>
	...
	<view class="content" :style="'height:'+contentHeight+'%'">
		...
		<view v-for="(item,index) in productList" :key="index" class="card">
			<navigator :url="'/surveyPages/carReport/drive/drive?driveId='+ item.driveId">
				<view style="font-weight: bold;margin-left: 30rpx;">
					<text>{{item.driveLicense}}</text>
				</view>
				<view style="color: #999999;margin-left: 30rpx;">
					<text>行车ID:{{item.driveId}}</text>
				</view>
	...
	export default {
		...
		methods: {
			getList() {
				let _this = this;
				uni.request({
					url: _this.apiServer + "/api/get/getAll/drive",
					method: 'POST',
					header: {
