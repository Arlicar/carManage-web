		...
		<view class="uni-flex uni-column">
			<view class="introBody" v-for="(item,index) in productList" :key="index">
				...
				<view class="uni-flex uni-row">
					...
					<view v-if="item.status==0" class="state" style="color: #ffc940;font-weight:600;font-size: 35rpx;">
						<text>已派出</text>
					</view>
					<view v-if="item.status==1" class="state" style="color: #87e9ff;font-weight:600;font-size: 35rpx;">
						<text>未派出</text>
					</view>
					<view v-if="item.status==2" class="state" style="color: #d295de;font-weight:600;font-size: 35rpx;">
						<text>维修中</text>
					</view>
					<view v-if="item.status==3" class="state" style="color: #7cff87;font-weight:600;font-size: 35rpx;">
						<text>保养中</text>
					</view>
					<view v-if="item.status==4" class="state" style="color: #a25454;font-weight:600;font-size: 35rpx;">
						<text>出借中</text>
					</view>
				</view>
				</uni-card>
				<uni-card style="width: 100%;">
				<text class="intro">开始使用时间：{{item.begin | formatDate}}</text>
				</uni-card>
				<uni-card style="width: 100%;">
				<view class="uni-flex uni-row">
				...
				<uni-card style="width: 100%;">
				<text class="intro">上次保养时间：{{item.lastProtectTime | formatDate}}</text>
				</uni-card>
			</view>
		</view>
	</view>
