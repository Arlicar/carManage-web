<template>
	<!--manage- 车辆管理 -->
	<view class="content" :style="'position:'+contentPosition+';'+'height:'+totalHeight+'%;'" >
		<!-- 头部区域 -->
		<view class="top">
	...
	export default {
		...
		methods: {
			getList() {
				...
				uni.request({
					...
					success: res => {
						...
						_this.totalHeight = 62 + res.data.data.length*20
					},
					fail: res => {
						console.log("发生了错误");
					},
			...
			confirmDialog() {
				...
				this.totalHeight = this.totalHeight-40
			},
			confirmDialog2() {
			...
			dialogInputConfirm() {
				...
				uni.uploadFile({
					...
					name: "licenseImg",//这里名字要对上
					formData: {
						license:_this.formData.license,
						peopleNum:_this.formData.peopleNum,
						emission: _this.formData.emission,
