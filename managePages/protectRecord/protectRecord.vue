<template>
	...
	<view class="content" :style="'height:'+contentHeight+'%'">
		...
		<view  v-for="(item,index) in productList" :key="index" class="card">
			<navigator :url="'/managePages/protectRecord/protectDetail/protectDetail?protectId='+ item.protectId" >
				<view style="font-weight: bold;margin-left: 30rpx;">
					<text>{{item.protectMsg}}</text>
				</view>
				<view style="color: #999999;margin-left: 30rpx;">
					<text>{{item.name}}</text>
				</view>
				<view class="uni-flex uni-row" style="color: #999999;margin-left: 30rpx;justify-content: space-between; margin-right: 60rpx;">
					<text>{{item.protectTime | formData}}</text>
