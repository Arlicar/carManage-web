<template>
	...
	<view class="content" :style="'height:'+contentHeight+'%'">
		...
		<view v-for="(item,index) in protectList" :key="index" class="card">
			<navigator :url="'protectDetail/protectDetail?protectId='+item.protectId">
	...
	export default {
		...
		methods: {
			getList() {
				let _this = this;
				uni.request({
					url: _this.apiServer + "/api/get/getAll/protectDeclare",
					method: 'POST',
					header: {
					...
					success: res => {
						...
						this.protectList = res.data.data;
